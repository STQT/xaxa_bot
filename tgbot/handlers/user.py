...
from tgbot.db.db_cmds import create_user
...
async def user_start(m: Message, status):
    if status:
        await m.answer(_("Assalomu alaykum!\nIltimos tilni tanlang 👇"), reply_markup=await lang_btns(False))
        await UserLangState.get_lang.set()
    else:
        await m.answer(_("Assalomu alaykum siz kimsiz?"), reply_markup=await main_menu_btns(False))
...
async def get_lang(c: CallbackQuery):
    ...
    await c.message.edit_text(_("Assalomu alaykum siz kimsiz?", locale=c.data),
                              reply_markup=await main_menu_btns(c.data))
    await UserMenuState.get_menu.set()
...
async def get_code(m: Message, state: FSMContext):
    ...
    if data["type"] == "seller":
        await m.answer(_("Sizning magaziningiz qaysi tumanda joylashgan? 🏬"))
...
async def get_region(m: Message, state: FSMContext):
    ...
    await m.answer(_("Iltimos manzilni kiriting"))
...
async def get_address(m: Message, state: FSMContext):
    ...
    await m.answer(_("Tez orada agentimiz siz bilan bog'lanadi 👨‍💻"
                     ...
                     "\nva kerakli bo'lgan zakazlarni qabul qiling! 📋"), reply_markup=await main_menu_btns())
    await UserMenuState.get_menu.set()
...
async def back(c: CallbackQuery):
    ...
    await c.message.answer(_("Assalomu alaykum siz kimsiz?"), reply_markup=await main_menu_btns(False))
    await UserMenuState.get_menu.set()
...
def register_user(dp: Dispatcher):
    ...
    dp.register_message_handler(get_region, state=UserSellerState.get_region)
